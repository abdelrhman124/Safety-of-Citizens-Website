...
const SewerageAuthority = require('../models/SewerageAuthority');
...
router.post("/", verifyToken.verifytoken, upload, async (req, res) => {
    ...
    const lastComplaint = await SewerageAuthority.findOne().sort({code: -1});
    ...
    const temp = 3;
    const newComplaint = new SewerageAuthority({
