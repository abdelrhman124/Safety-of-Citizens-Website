...
const RoadsAndBridgesAuthority = require('../models/RoadsAndBridgesAuthority');
...
router.post("/", verifyToken.verifytoken, upload, async (req, res) => {
    ...
    // check if there is a complaint
    const lastComplaint = await RoadsAndBridgesAuthority.findOne().sort({code: -1});
    ...
    const temp = 2;
    const newComplaint = new RoadsAndBridgesAuthority({
